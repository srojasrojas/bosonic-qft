...
- **`qft_eq27.py`**: Streamlined solver for λ_k parameters satisfying equation 27 (complex Hadamard matrix condition). Features a multi-phase workflow: BFGS warm-up → Differential Evolution multi-run → L-BFGS-B refinement → rational π approximation. Multi-run mode is default, saving only mathematically verified solutions.
